...
func Load(path string) (*Config, error) {
	...
	if cfg.Database.SQLitePath == "" {
		cfg.Database.SQLitePath = "./data/supplycore.db"
	}
	if cfg.Database.PostgresDSN == "" {
		cfg.Database.PostgresDSN = "host=127.0.0.1 user=postgres password=postgres dbname=supplycore port=5432 sslmode=disable TimeZone=Asia/Shanghai"
	}
	...
	if cfg.Storage.PublicBaseURL == "" {
		if cfg.Storage.Driver == "minio" {
			cfg.Storage.PublicBaseURL = "http://127.0.0.1:9100/supplycore"
		} else {
			cfg.Storage.PublicBaseURL = "http://127.0.0.1:8092/uploads"
		}
	}
